...
<head>
    ...
    <title>Controle Vendas | Condições Pagamento</title>
        ...
        <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="background-color:<?= $cor?>!important">
            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item d-none d-sm-inline-block">
                    <a class="nav-link nav-personalizate" >Condições de Pagamento</a>
                </li>
            </ul>
        </nav>
                    ...
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <button type="button" class="btn btn-inserir btn-personalizate" data-toggle="modal"
                                data-target="#modalCadastroCondicaoPagamento">
                                Inserir
                            </button>
                            

                            <!-- CADASTRO DE USUÁRIOS -->
                            <div class="modal fade" id="modalCadastroCondicaoPagamento" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header" style="background-color: <?=$cor ?>!important;">
                                            <h5 class="modal-title" style="color: white!important;"
                                                id="exampleModalLabel">Cadastro de Condição de Pagamento</h5>
                                                <!-- Função X -->
                                        ...
                                        <?php	
                                            if(!isset($conpagcodigo))
                                                $conpagcodigo 	= isset($_GET["conpagcodigo"]) ? $_GET["conpagcodigo"] : null;
                                            if(!isset($conpagdescri))
                                                $conpagdescri 	= isset($_GET["conpagdescri"]) ? $_GET["conpagdescri"] : null;
                                            if(!isset($arq))
                                                $arq 	= isset($_GET["arq"]) ? $_GET["arq"] : "InsereCondicaoPagamento.php";
                                        ?>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form method="post" action="<?php echo $arq; ?>" class="row">
                                                ...
                                                <div class="col-12 col-md-12 col-sm-6">
                                                    <div>
                                                        <label for="conpagdescri" class="col-sm-10 col-form-label">
                                                            Descrição
                                                        </label>
                                                    </div>
                                                    <div class="col-sm-10">
                                                        <input type="text" class="form-control" id="conpagdescri" name="conpagdescri" value="<?php echo $conpagdescri ?>">
                                                    </div>
                                                </div>

                                                <input type="hidden" name="conpagcodigo" value="<?php echo "$conpagcodigo" ?>">
                                ...
                                <tbody>
                                    <?php
                                        while($row = mysqli_fetch_array($query)){
                                            $conpagcodigo = $row['conpagcodigo'];
                                            $conpagdescri = $row['conpagdescri'];?>
                                            <tr>
                                                <td><?= $conpagcodigo ?></td>
                                                <td><?= $conpagdescri ?></td>
                                                <td><a href="DeletaCondicaoPagamento.php?conpagcodigo=<?php echo $row['conpagcodigo']; ?>" type="" class="btn btn-sm btn-danger fas fa-trash-alt" name="excluir" id="excluir"></a>
                                                <a href="DeletaCondicaoPagamento.php?conpagcodigo=<?php echo $row['conpagcodigo']; ?>" type="" class="btn btn-sm btn-danger fas fa-trash-alt" name="excluir" id="excluir"  
                                                data-toggle="modal" data-target="#modalCadastroCondicaoPagamento"></td>
                                            </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
